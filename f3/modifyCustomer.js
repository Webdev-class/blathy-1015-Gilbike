...
function isAddress(address) {
  ...
  const isCityValid = typeof address.city == "string";
  const isStreetValid = typeof address.street == "string";
  const isHouseValid = typeof address.house == "number";
  return isCityValid && isStreetValid && isHouseValid;
}

function isCustomer(name, age, address, newsLetter) {
  return (
    customers.filter((customer) => {
      return customer.name == name && customer.age == age && customer.address == address && customer.newsLetter == newsLetter;
    }).length == 1
  );
}
function modifyCustomer(customer) {
  if (customer == undefined) return false;
  if (!isAddress(customer.address) || isCustomer(customer.name, customer.age, customer.address, customer.newsLetter)) return false;
  const id = customers.findIndex((c) => c.id == customer.id);
  const newCustomers = [...customers];
  newCustomers[id] = customer;
  return newCustomers;
}
